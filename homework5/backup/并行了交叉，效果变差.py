...
class GeneticAlgTSP:
    ...
    def crossover_wrapper(self, args):
        ...
        with Pool(processes=8) as pool:
            for iteration in range(num_iterations):
                ...
                for i, individual in enumerate(self.population):
                    fitnesses[i] = calculate_fitness_numba(self.cities, individual)
                new_population_indices = selection(np.arange(len(self.population)), fitnesses, self.population_size)
                # 根据索引，选择在大小范围内的新种群
                ...
                # 对每对父代个体进行交叉操作,并行版
                crossover_data = [
                    (new_population[i], new_population[i + 1 if i + 1 < self.population_size else 0]) for i
                    in range(0, self.population_size, 2)]
                crossover_results = pool.map(self.crossover_wrapper, crossover_data)
                size = self.population_size + len(crossover_results)
                next_generation = np.empty(size, dtype=np.ndarray)
                # 将原始种群复制到新数组的前半部分
                for i in range(self.population_size):
                    next_generation[i] = new_population[i]
                # 执行交叉操作并填充新生成的子代到数组的后半部分
                crossover_results = pool.map(self.crossover_wrapper, crossover_data)
                for i, child in enumerate(crossover_results):
                    next_generation[self.population_size + i] = child
                # 串行版,初始化未1.5倍，向上取整
                # if self.population_size % 2 == 1:
                #     # 奇数
                #     size = int((self.population_size + 1) * 1.5)
                # else:
                #     size = int(self.population_size * 1.5)
                # next_generation = np.empty(size, dtype=np.ndarray)
                # cid = 0
                ...
                #     next_generation[i + 1] = parent2
                #     next_generation[self.population_size + cid] = child
                #     cid += 1
...
def main():
    ...
    best_path, best_iteration, best_ever_fitness, run_time = ga_tsp.iterate(100)
